...
const BoxBuyDetailComponent = () => {
  ...
  useEffect(() => {
    async function fetchPrice() {
      ...
      const packPrice = await saleContract.methods.packPrices(0).call()
      setPrice(packPrice)
    }
  ...
  useEffect(() => {
    async function fetchWhitelistedStatus() {
      const web3 = new Web3(Web3.givenProvider)
      const saleContract = new web3.eth.Contract(NftSale.abi as AbiItem[], getNftSaleAddress())
      const retv = await saleContract.methods.whitelisted(account, 0).call()
      console.log(retv)
  ...
  return (
    <div>
      <BoxTitle style={{ color: isDark ? 'white' : '' }}>Common Pack</BoxTitle>
      <RemainingAmount style={{ color: isDark ? 'white' : '' }}>
        <p>Max Supply 62300</p>
        <p>1 Common Cow + 1 Common Bull + 1 Land</p>
      </RemainingAmount>
      <BoxPrice
        style={{
