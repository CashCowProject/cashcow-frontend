...
const StakeItem = ({data, index}) => {
  ...
  const fetchNft = useCallback(async ()=>{
    if (!data || !data.tokenId)
        return;
    let isAIR = false;
    if(data.contractAddress === getHappyCowAddress())
      isAIR = false;
    else if(data.contractAddress === getAirNftAddress())
      isAIR = true;
    let tokenURI;
    if(!isAIR)
      tokenURI = await happyCowsContract.methods.tokenURI(data.tokenId).call();
    else
