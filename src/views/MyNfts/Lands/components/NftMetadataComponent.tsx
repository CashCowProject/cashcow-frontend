...
const NftMetadataComponent = ({ tokenId }: NftDataLeftComponentInterface) => {
  ...
  const fetchNftInfo = useCallback(async () => {
    ...
    const _image =
      ...
      LAND_RARITY[parseInt(attrs.rarity)] +
      '-' +
      LAND_KIND[parseInt(attrs.landType)] +
      '.png'
    setNftImage(_image)
  ...
  const saleActionHandler = async (_tokenId: string) => {
    try {
      ...
      await marketContract.methods
        .createMarketItem(getLandNftAddress(), _tokenId, toBN(toWei(salePrice.toString())))
        .send({ from: account })
      setLoading(false)
      history.push('/lands')
  ...
  return (
    <Container style={{ background: isDark ? '#27262c' : '' }}>
      ...
      <MetadataContainer>
        ...
        <NftInfo>
          <TitleContainer style={{ color: isDark ? 'white' : '' }}>
            {kindName ? kindName : 'land'} #{tokenId}
          </TitleContainer>
          <AttributesContainer
            style={{
