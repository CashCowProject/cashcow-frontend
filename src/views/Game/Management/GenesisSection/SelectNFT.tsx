...
const SelectNFT = ({ isOpen, closeDialog, fetchOriginalUserGenesis }) => {
  ...
  const genesisContract = new web3.eth.Contract(Genesis.abi as AbiItem[], temporalGenesisContract);
  ...
  const handleAddNft = async (_tokenId: string) => {
    ...
    try {
      // setLoading(true);
      await genesisContract.methods.approve(temporalFarmingContract, _tokenId).send({ from: account });
      await farmingContract.methods.depositGenesis(_tokenId).send({ from: account })
