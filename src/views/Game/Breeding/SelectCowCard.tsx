...
export const SelectCowCard = ({ nftEachItem, idx, handleSelectNft }) => {
    ...
    return (
        <NftEachItemContainer style={{ background: isDark ? '#383740' : '' }}>
            <ItemTop>
                ...
                <NftImageContainer>
                <NftImage style={{ backgroundImage: `url(${nftEachItem.image})` }} onClick={() => handleSelectNft(nftEachItem.tokenId, nftEachItem.image)} key={nftEachItem.tokenId + "_" + idx}/>
                {/* <img src={nft.image} alt="" style={{ cursor: 'pointer', width: "30%", marginRight: '8px', marginTop: '10px' }} onClick={() => actionHandler(nft.tokenId)} /> */}
              </NftImageContainer>
              <ItemMetaData>
                <span style={{ color: isDark ? 'white' : 'black' }}>Cow #{nftEachItem.tokenId}</span>
              </ItemMetaData>
            </ItemTop>
