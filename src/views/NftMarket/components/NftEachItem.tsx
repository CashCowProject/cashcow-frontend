            ...
            <NftEachItemContainer style={{background : isDark ? '#27262c' : ''}}>
                ...
                <ItemBottom>
                    <ItemTitle style={{color: isDark ? 'white' : ''}}>
                        Sale Price
                        <span> ≈ $ {getNumberSuffix(Math.floor(milkPrice * parseInt(fromWei(nftEachItem.price, 'ether')) * 100) / 100)}</span>
                    </ItemTitle>
                    <ItemValue>
                        <ItemValueText style={{color: isDark ? 'white' : ''}}>{getNumberSuffix(fromWei(nftEachItem.price, 'ether'))}</ItemValueText>
